...
# designs
def get_similar_designs(gender, event, dress_type, article_type, img_url, height, hip_size):
    skin_col = -1
    height_threshold = 0
    hip_size_threshold = 0
    idx_rec_modified = []
    if img_url != '':
    ...
    if hip_size !="":
        if hip_size >= 70:
            hip_size_threshold = 1
    print(skin_col)
    idx = dal.get_base_design(gender, event, dress_type, article_type, skin_col, height_threshold, hip_size_threshold)
    if idx != -1:
        print('idx', idx)
