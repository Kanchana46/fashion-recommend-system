...
def get_base_design(gender, event, dress_type, article_type, skin_color, height_threshold, hip_size_threshold):
    print(height_threshold, hip_size_threshold)
    smilar_designs = []
    if gender != '':
        docs = designs. \
            where("gender", "==", gender)
    ...
    if height_threshold != '':
        docs = docs. \
            where("height_threshold", "==", int(height_threshold))
    if hip_size_threshold != '':
        docs = docs. \
            where("hipSize_threshold", "==", int(hip_size_threshold))
    if skin_color != -1:
        docs = docs. \
...
for index, row in df.iterrows():
    ...
    doc_ref.set({
        ...
        'height_threshold': row.height_threshold,
        'hipSize_threshold': row.hipSize_threshold,
